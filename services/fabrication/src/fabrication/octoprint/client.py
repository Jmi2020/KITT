...
class OctoPrintClient:
    ...
    async def upload_file(self, gcode_path: Path, select: bool = True) -> Dict[str, Any]:
        files = {"file": (gcode_path.name, gcode_path.read_bytes(), "application/gcode")}
        data = {"select": str(select).lower(), "print": "false"}
        response = await self._request("post", "/api/files/local", files=files, data=data)
        return response.json()

    async def set_tool_temperature(self, target: int) -> None:
        await self._request("post", "/api/printer/tool", json={"command": "target", "targets": {"tool0": target}})

    async def set_bed_temperature(self, target: int) -> None:
        await self._request("post", "/api/printer/bed", json={"command": "target", "target": target})
    ...
    async def pause_job(self, action: str = "pause") -> None:
        await self._request("post", "/api/job", json={"command": "pause", "action": action})
