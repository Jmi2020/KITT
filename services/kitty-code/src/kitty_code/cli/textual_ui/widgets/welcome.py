...
class WelcomeBanner(Static):
    FLASH_COLOR = "#FFFFFF"
    # Coder gradient: cyan -> blue -> purple -> magenta -> pink
    TARGET_COLORS = ("#00D4FF", "#0088FF", "#6644FF", "#AA22FF", "#FF44AA")
    BORDER_TARGET_COLOR = "#6644FF"
    ...
    def _initialize_static_line_suffixes(self) -> None:
        self._static_line1_suffix = (
            f"{self.LOGO_TEXT_GAP}[b]Kitty Code v{__version__}[/]"
        )
    ...
    def _build_line(self, line_idx: int, color: str) -> str:
        ...
        # Cat face pattern with pointed ears
        patterns = [
            # Line 1: Ears (pointed triangles)
            f"[{color}]{B}[/]{S}{S}{S}{S}{S}[{color}]{B}[/]{S}{self._static_line1_suffix}",
            # Line 2: Upper head with ear tips
            f"[{color}]{B}{B}[/]{S}{S}{S}[{color}]{B}{B}[/]{S}{self._static_line2_suffix}",
            # Line 3: Eyes (full head width with gaps for eyes)
            f"[{color}]{B}[/]{S}[{color}]{B}[/]{S}[{color}]{B}[/]{S}{self._static_line3_suffix}",
            # Line 4: Full face
            f"[{color}]{B}{B}{B}{B}{B}[/]{S}",
            # Line 5: Chin/whiskers
            f"{S}[{color}]{B}{B}{B}[/]{S}{S}{self._static_line5_suffix}",
        ]
