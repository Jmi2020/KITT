...
Timestamp & Freshness: The wrapper now injects the live UTC time and, when heuristics flag a prompt as time-sensitive (`services/brain/src/brain/routing/freshness.py`), adds a lightweight warning that training data may be stale and the agent must confirm via tools like `web_search`. This keeps local runs grounded without bloating tokens.
